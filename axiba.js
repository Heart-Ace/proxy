...
const jsname = '企鹅读书'
const $ = Env(jsname)
const COOKIE = $.isNode() ? require("./qqreadCOOKIE1") : "";
...
let task, tz, kz, config = '', CASH = '10', COOKIES_SPLIT = '' ;
...
function showmsg() {
  return new Promise(async resolve => {
    if (BOX != 1) {
      ...
      if (notifyInterval == 2 && (nowTimes.getHours() === 12 || nowTimes.getHours() === 22) && (nowTimes.getMinutes() >= 0 && nowTimes.getMinutes() <= 10)) {
        $.msg(O, "", tz);
      }
      ...
      if (notifyttt == 1 && $.isNode() && (nowTimes.getHours() === 12 || nowTimes.getHours() === 22) && (nowTimes.getMinutes() >= 0 && nowTimes.getMinutes() <= 10))
